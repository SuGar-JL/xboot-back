...
public class SystemLogAspect {
    ...
    @Value("${xboot.logRecord.es}") //@Value从配置文件.yml中获取xboot下的logRecord下的es的值（false)
    ...
    public void after(JoinPoint joinPoint){
        try {
            String username = "";
            String description = getControllerMethodInfo(joinPoint).get("description").toString();//获取方法信息，（后面定义的方法）
            Map<String, String[]> logParams = request.getParameterMap();//获取请求参数列表
            String principal = SecurityContextHolder.getContext().getAuthentication().getPrincipal().toString();//////
            // 判断允许不用登录的注解
            if("anonymousUser".equals(principal)&&!description.contains("短信登录")){//anonymousUser：匿名用户
                return;
            }
    ...
    public static Map<String, Object> getControllerMethodInfo(JoinPoint joinPoint) throws Exception{
        ...
        Map<String, Object> map = new HashMap<String, Object>(16);//////initialcapacity：初始容量
        //获取目标类名
